...
class AAARule:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.action = (
            action
            if action is None or isinstance(action, AAARuleAction)
            else AAARuleAction(**action)
        )
        self.condition = (
            condition
            if condition is None or isinstance(condition, AAARuleCondition)
            else AAARuleCondition(**condition)
        )
...
class Details:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.shared_with = (
            shared_with
            if shared_with is None or isinstance(shared_with, SharedWith)
            else SharedWith(**shared_with)
        )
        ...
        self.succeeded_users = (
            succeeded_users
            if succeeded_users is None or isinstance(succeeded_users, list)
            else json.loads(succeeded_users)
        )
        self.failed_users = (
            failed_users
            if failed_users is None or isinstance(failed_users, list)
            else json.loads(failed_users)
        )
        ...
        self.new_profile = (
            new_profile
            if new_profile is None or isinstance(new_profile, Profile)
            else Profile(**new_profile)
        )
        ...
        self.matched_rule = (
            matched_rule
            if matched_rule is None or isinstance(matched_rule, AAARule)
            else AAARule(**matched_rule)
        )
        self.request = (
            request
            if request is None or isinstance(request, AAARequest)
            else AAARequest(**request)
        )
...
class WorkflowV2:
    ...
    def __init__(
        ...
        *,
        ...
        step_configuration: Optional[
            List[Union[Dict[str, Any], WorkflowV2StepConfiguration]]
        ] = None,
        **kwargs,
    ) -> None:
        self.id = id
...
class Entity:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.workflow_v2 = (
            WorkflowV2(**workflow_v2) if isinstance(workflow_v2, dict) else workflow_v2
        )
        self.account_type_role = (
            AccountTypeRole(**account_type_role)
            if isinstance(account_type_role, dict)
            else account_type_role
        )
