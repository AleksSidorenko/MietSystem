...
def _parse_web_class_objects(kwargs) -> None:
    ...
    for blocks_name in ["blocks", "user_auth_blocks"]:
        ...
        if (
            blocks is not None
            and isinstance(blocks, Sequence)
...
def _next_cursor_is_present(data) -> bool:
    ...
    present = (
        "next_cursor" in data
        and data["next_cursor"] is not None
...
def _to_v2_file_upload_item(upload_file: Dict[str, Any]) -> Dict[str, Optional[Any]]:
    ...
    if file is not None:
        if isinstance(file, (str, os.PathLike)):  # filepath
            ...
            raise SlackRequestError(
                "file parameter must be any of filepath, bytes, and io.IOBase"
            )
    ...
    if data is None:
        raise SlackRequestError(
            f"File content not found for filename: {filename}, title: {title}"
        )
