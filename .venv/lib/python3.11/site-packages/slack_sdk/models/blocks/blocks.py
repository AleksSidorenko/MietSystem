...
class ImageBlock(Block):
    ...
    def attributes(self) -> Set[str]:  # type: ignore[override]
        return super().attributes.union(
            {"alt_text", "image_url", "title", "slack_file"}
        )
            ...
            if isinstance(title, str):
                ...
                if title.get("type") != PlainTextObject.type:
                    raise SlackObjectFormationError(
                        f"Unsupported type for title in an image block: {title.get('type')}"
                    )
                ...
                raise SlackObjectFormationError(
                    f"Unsupported type for title in an image block: {type(title)}"
                )
        if slack_file is not None:
            self.slack_file = (
                slack_file
                if slack_file is None or isinstance(slack_file, SlackFile)
                else SlackFile(**slack_file)
            )
    ...
    def _validate_image_url_length(self):
        return (
            self.image_url is None or len(self.image_url) <= self.image_url_max_length
        )
...
class VideoBlock(Block):
    ...
    @JsonValidator(
        f"author_name attribute cannot exceed {author_name_max_length} characters"
    )
    def _validate_author_name_length(self):
        return (
            self.author_name is None
            or len(self.author_name) < self.author_name_max_length
        )
