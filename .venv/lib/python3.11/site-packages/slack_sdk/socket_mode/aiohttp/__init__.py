...
import aiohttp
from aiohttp import ClientConnectionError, ClientWebSocketResponse, WSMessage, WSMsgType
                ...
                try:
                    ...
                    if message is not None:
                        if self.logger.level <= logging.DEBUG:
                            ...
                            message_type = (
                                m_type.name if m_type is not None else message.type
                            )
                            message_data = message.data
                            if isinstance(message_data, bytes):
                                message_data = message_data.decode("utf-8")
                            if (
                                message_data is not None
                                and isinstance(message_data, (str, bytes))
                                and len(message_data) > 0
                            ):
