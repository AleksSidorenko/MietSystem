...
class SQLAlchemyInstallationStore(InstallationStore):
    ...
    def find_installation(
        ...
        *,
        ...
        no_bot_token_installation = (
            installation is not None and installation.bot_token is None
        )
        should_find_bot_installation = (
            has_user_installation or no_bot_token_installation
        )
...
class AsyncSQLAlchemyInstallationStore(AsyncInstallationStore):
    ...
    @classmethod
    def build_installations_table(cls, metadata: MetaData, table_name: str) -> Table:
        return SQLAlchemyInstallationStore.build_installations_table(
            metadata, table_name
        )
    ...
    async def async_find_installation(
        ...
        *,
        ...
        async with self.engine.connect() as conn:
            result: object = await conn.execute(query)
            for row in result.mappings():  # type: ignore[attr-defined]
                installation = SQLAlchemyInstallationStore.build_installation_entity(
                    row
                )
