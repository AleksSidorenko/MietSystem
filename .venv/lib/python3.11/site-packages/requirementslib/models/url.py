...
class URI(ReqLibBaseModel):
    ...
    path: Optional[str] = Field(
        "", description="The url path, e.g. `/path/to/endpoint`"
    )
    ...
    ref: Optional[str] = Field(
        "", description="VCS ref this URI points at, if available"
    )
    ...
    def get_parsed_url(cls, url):
        ...
        if "@" in url and "#" in url:
            ...
            if "#" in maybe_auth and (
                not parsed_url.host or "." not in parsed_url.host
            ):
    ...
    if name_with_extras:
        ...
        if parsed_dict["fragment"] is not None:
            ...
            if fragment.startswith("egg="):
                ...
                name_with_extras = "{0}{1}".format(
                    name, extras_to_string(parsed_extras)
                )
        elif (
            parsed_dict.get("path") is not None
            and "&subdirectory" in parsed_dict["path"]
        ):
