...
class Dependency(ReqLibBaseModel):
    ...
    def from_requirement(cls, req, parent=None):
        ...
        return cls(
            ...
            extras=(
                tuple(sorted(set(req.extras))) if req.extras is not None else req.extras
            ),
            requirement=req,
...
class ReleaseUrlCollection(BaseModel, Sequence):
    ...
    def latest(self) -> Optional[ReleaseUrl]:
        ...
        return next(
            iter(
                sorted(self.urls, key=operator.attrgetter("upload_time"), reverse=True)
            )
        )
...
class Package(BaseModel):
    ...
    def get_latest_lockfile(self):
        ...
        for sub_dep_name, specset in constraints.items():
            ...
            try:
                version = next(
                    iter(specset.filter((r.version for r in sorted_releases)))
                )
...
def get_package_from_requirement(req):
    ...
    if is_pinned_requirement(req):
        ...
        sorted_releases = list(
            sorted(
                pkg.releases, key=operator.attrgetter("parsed_version"), reverse=True
            )
        )
