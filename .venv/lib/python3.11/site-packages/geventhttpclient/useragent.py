...
class UserAgent:
    ...
    def urlopen(
        ...
        **kw,
        ...
        max_redirects = (
            int(max_redirects) if max_redirects is not None else self.max_redirects
        )
        ...
        for retry in range(max_retries + 1):
            ...
            for _ in range(max_redirects + 1):
                ...
                try:
                    ...
                    e.response = resp
                    e.request = req
                    e.http_log = self._conversation_str(
                        req.url, resp, payload=req.payload
                    )
                    resp.release()
                    last_error = self._handle_error(e, url=req.url)
                    break  # Continue with next retry
                ...
                e = RetriesExceeded(
                    url, f"Redirection limit reached ({self.max_redirects})"
                )
    ...
    def _make_request(
        self, url, method="GET", headers=None, payload=None, params=None, files=None
    ):
...
def _make_request(
    ...
    return request_type(
        url, method=method, headers=headers, payload=payload, params=params
    )
