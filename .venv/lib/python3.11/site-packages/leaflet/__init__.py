...
if SCALE is True:
    ...
    raise ImproperlyConfigured(
        "LEAFLET_CONFIG['SCALE'] must be True, False, None, 'metric', 'imperial' or 'both'."
    )


SPATIAL_EXTENT = app_settings.get("SPATIAL_EXTENT")
if (
    SPATIAL_EXTENT is not None
    and not isinstance(SPATIAL_EXTENT, (tuple, list))
    or len(SPATIAL_EXTENT) != 4
):
    raise ImproperlyConfigured(
        _("Spatial extent should be a tuple (minx, miny, maxx, maxy)")
    )
...
if (
    SRID
    and TILES_EXTENT
    and (TILES_EXTENT[2] - TILES_EXTENT[0] > TILES_EXTENT[3] - TILES_EXTENT[1])
):
    raise ImproperlyConfigured(
        "Landscape tiles extent not supported (%s)." % (TILES_EXTENT,)
    )


DEFAULT_CENTER = app_settings["DEFAULT_CENTER"]
if DEFAULT_CENTER is not None and not (
    isinstance(DEFAULT_CENTER, (list, tuple)) and len(DEFAULT_CENTER) == 2
):
    raise ImproperlyConfigured(
        "LEAFLET_CONFIG['DEFAULT_CENTER'] must be an list/tuple with two elements - (lat, lon)"
    )


DEFAULT_ZOOM = app_settings["DEFAULT_ZOOM"]
if DEFAULT_ZOOM is not None and not (
    isinstance(DEFAULT_ZOOM, int) and (1 <= DEFAULT_ZOOM <= 24)
):
    raise ImproperlyConfigured(
        "LEAFLET_CONFIG['DEFAULT_ZOOM'] must be an int between 1 and 24."
    )


DEFAULT_PRECISION = app_settings["DEFAULT_PRECISION"]
if DEFAULT_PRECISION is not None and not (
    isinstance(DEFAULT_PRECISION, int) and (4 <= DEFAULT_PRECISION <= 12)
):
    raise ImproperlyConfigured(
        "LEAFLET_CONFIG['DEFAULT_PRECISION'] must be an int between 4 and 12."
    )


PLUGINS = app_settings["PLUGINS"]
if not (
    isinstance(PLUGINS, dict) and all([isinstance(el, dict) for el in PLUGINS.values()])
):
...
_forms_js = ["leaflet/draw/leaflet.draw.js", "leaflet/leaflet.forms.js"]
if SRID:
    _forms_js += ["leaflet/proj4.js", "leaflet/proj4leaflet.js", "proj4js/%s.js" % SRID]
