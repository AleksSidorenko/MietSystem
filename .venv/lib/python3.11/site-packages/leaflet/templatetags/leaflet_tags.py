...
def leaflet_map(
    ...
    djoptions = dict(
        ...
        layers=[
            (force_str(label), url, attrs)
            for (label, url, attrs) in instance_app_settings.get("TILES")
        ],
        overlays=[
            (force_str(label), url, attrs)
            for (label, url, attrs) in instance_app_settings.get("OVERLAYS")
        ],
        attributionprefix=force_str(
            instance_app_settings.get("ATTRIBUTION_PREFIX"), strings_only=True
        ),
...
def leaflet_draw_i18n(context):
    return {
        ...
        "draw_handlers_polygon_tooltip_end": _(
            "Click first point to close this shape."
        ),
        "draw_handlers_polyline_error": _(
        ...
        "edit_handlers_edit_tooltip_text": _(
...
def leaflet_json_config():
    ...
    if SPATIAL_EXTENT is not None:
        ...
        settings_as_json["SPATIAL_EXTENT"] = {
            "xmin": xmin,
            "ymin": ymin,
            "xmax": xmax,
            "ymax": ymax,
