...
def get_field_names(model):
    ...
    for field in fields:
        ...
        if (
            field.model != model
            and field.model._meta.concrete_model == model._meta.concrete_model
        ):
            continue
...
class TestAutodiscover(ModeltranslationTestBase):
    ...
    def _pre_setup(self):
        ...
        new_installed_apps = django_settings.INSTALLED_APPS + (
            "modeltranslation.tests.test_app",
        )
    ...
    @reload_override_settings(
        MODELTRANSLATION_TRANSLATION_FILES=(
            "modeltranslation.tests.project_translation",
        )
    )
    ...
    @reload_override_settings(
        MODELTRANSLATION_TRANSLATION_FILES=(
            "modeltranslation.tests.test_app.translation",
        )
    )
...
class ModeltranslationTest(ModeltranslationTestBase):
    ...
    @parameterized.expand(
        [
            (
                {"title": "DE"},
                ["title"],
                {"title": "DE", "title_de": "DE", "title_en": None},
            ),
            (
                {"title_de": "DE"},
                ["title"],
    ...
    def test_save_original_translation_field(
        self, field_values, update_fields, expected_db_values
    ):
    ...
    @parameterized.expand(
        [
            (
                {"title": "EN"},
                ["title"],
                {"title": "EN", "title_de": None, "title_en": "EN"},
            ),
            (
                {"title_en": "EN"},
                ["title"],
    ...
    def test_save_active_translation_field(
        self, field_values, update_fields, expected_db_values
    ):
        with override("en"):
            obj = models.TestModel.objects.create(title="old")
...
class FallbackTests(ModeltranslationTestBase):
    ...
    def test_settings(self):
        ...
        # Whole dict
        with reload_override_settings(
            MODELTRANSLATION_FALLBACK_LANGUAGES=self.test_fallback
        ):
...
class ForeignKeyFieldsTest(ModeltranslationTestBase):
    ...
    def test_reverse_relations(self):
        ...
        fk_inst_de.save()
        fk_inst_en = self.model(
            title_en="f_title_en", title_de="f_title_de", test_en=instance
        )
        fk_inst_en.save()
    ...
    def test_reverse_lookup_with_filtered_queryset_manager(self):
        ...
        assert (
            models.FilteredTestModel._meta.base_manager.__class__ == MultilingualManager
        )
        ...
        assert (
            models.ForeignKeyFilteredModel._meta.base_manager.__class__
            == MultilingualManager
        )
...
class ManyToManyFieldsTest(ModeltranslationTestBase):
    ...
    def test_db_column_names(self):
        ...
        assert (
            ...
            == "tests_manytomanyfieldmodel_test"
        )

        field = meta.get_field("test_en")
        assert (
            field.remote_field.through._meta.db_table
            == "tests_manytomanyfieldmodel_test_en"
        )

        field = meta.get_field("test_de")
        assert (
            field.remote_field.through._meta.db_table
            == "tests_manytomanyfieldmodel_test_de"
        )

        field = meta.get_field("self_call_1")
        assert (
            field.remote_field.through._meta.db_table
            == "tests_manytomanyfieldmodel_self_call_1"
        )

        field = meta.get_field("self_call_1_en")
        assert (
            field.remote_field.through._meta.db_table
            == "tests_manytomanyfieldmodel_self_call_1_en"
        )

        field = meta.get_field("self_call_1_de")
        assert (
            field.remote_field.through._meta.db_table
            == "tests_manytomanyfieldmodel_self_call_1_de"
        )
        ...
        assert (
            field.remote_field.through._meta.db_table == "tests_customthroughmodel_en"
        )

        field = meta.get_field("through_model_de")
        assert (
            field.remote_field.through._meta.db_table == "tests_customthroughmodel_de"
        )

    def test_translated_models_instance(self):
        models.TestModel.objects.bulk_create(
            models.TestModel(
                title_en="m2m_test_%s_en" % i, title_de="m2m_test_%s_de" % i
            )
            for i in range(10)
        )
        ...
        testmodel_qs_1 = testmodel_qs.filter(
            title_en__in=["m2m_test_%s_en" % i for i in range(4)]
        )
        ...
        self_qs_1 = self_qs.filter(
            title_en__in=["m2m_test_%s_en" % i for i in range(6)]
        )
        self_qs_2 = self_qs.filter(
            title_en__in=["m2m_test_%s_en" % i for i in range(6, 10)]
        )
        ...
        assert (
            inst.through_model.through.objects.all().count() == testmodel_qs_2.count()
        )
        ...
        assert (
            inst.self_call_1.filter(
                pk__in=self_qs_2.values_list("pk", flat=True)
            ).count()
            == self_qs_2.count()
        )
        ...
        assert (
            manager.filter(
                through_model__title__in=testmodel_qs_2.values_list("title", flat=True)
            )
            .distinct()
            .count()
            == 1
        ...
        assert (
            manager.filter(
                through_model_en__title__in=testmodel_qs_2.values_list(
                    "title", flat=True
                )
            ).count()
            == 0
        ...
        assert (
            manager.filter(
                through_model_de__title__in=testmodel_qs_2.values_list(
                    "title", flat=True
                )
            )
            .distinct()
        ...
        trans_real.activate("en")
        assert (
            manager.filter(trans_through_model__in=testmodel_qs_1).distinct().count()
            == 1
        )
        assert manager.filter(trans_through_model_de__in=testmodel_qs_1).count() == 0
        assert (
            manager.filter(trans_through_model_en__in=testmodel_qs_1).distinct().count()
            == 1
        )
    ...
    def test_reverse_relations(self):
        ...
        assert (
            manager.filter(m2m_test_ref__title_en="inst_de_en").distinct().count() == 10
        )
        assert (
            manager.filter(m2m_test_ref_en__title="inst_en_de").distinct().count() == 10
        )
        ...
        assert (
            manager.filter(m2m_test_ref__title_de="inst_en_de").distinct().count() == 10
        )
        assert (
            manager.filter(m2m_test_ref_de__title="inst_de_en").distinct().count() == 10
        )
...
class OtherFieldsTest(ModeltranslationTestBase):
    ...
    def test_descriptors(self):
        ...
        inst.save()
        db_values = models.DescriptorModel.objects.raw_values(
            "normal", "trans_en", "trans_de"
        )[0]
...
class ModeltranslationTestRule2(ModeltranslationTestBase):
    ...
    def _test_field(
        self, field_name, value1_de, value1_en, value2, value3, deactivate=True
    ):
        field_name_de = "%s_de" % field_name
        field_name_en = "%s_en" % field_name
...
class UpdateCommandTest(ModeltranslationTestBase):
    ...
    def test_update_command_with_json_field(self):
        ...
        instance = models.OtherFieldsModel.objects.filter(pk=instance_pk).raw_values()[
            0
        ]
...
class TestManager(ModeltranslationTestBase):
    ...
    def test_filter_update(self):
        ...
        with override("de"):
            ...
            # Spanning works
            assert (
                2 == models.ManagerTestModel.objects.filter(title__endswith="e").count()
            )
            ...
            assert (
                2
                == models.ManagerTestModel.objects.filter(
                    title_en__contains="en"
                ).count()
            )
        ...
        assert (
            1
            == models.ManagerTestModel.objects.filter(title="en", title_en="en").count()
        )
    ...
    def test_q(self):
        ...
        assert (
            0
            == models.ManagerTestModel.objects.filter(Q(title="de") | Q(pk=42)).count()
        )
        assert (
            1
            == models.ManagerTestModel.objects.filter(Q(title="en") | Q(pk=42)).count()
        )
    ...
    def test_order_by_reset(self):
        ...
        assert (
            not qs.order_by().values("title").ordered
        ), "queryset is unexpectedly ordered"
    ...
    def test_values_fallback(self):
        ...
        self.assert_fallback(
            manager.values, "de", "title", transform=lambda x: x["title"]
        )
        self.assert_fallback(manager.values_list, "de", "title", flat=True)
        self.assert_fallback(
            manager.values_list, ("de", "", "de"), "title", "title_en", "title_de"
        )
        ...
        self.assert_fallback(
            manager.values, "fallback", "title", transform=lambda x: x["title"]
        )
        self.assert_fallback(
            manager.values_list, ("fallback", "fallback"), "title", "text"
        )
    ...
    def test_values(self):
        ...
        with override("de"):
            assert list(manager.values_list("title", "visits", named=True)) == [
                ("de", 0)
            ]
        ...
        assert list(manager.values_list()) == [
            (id1, "en", 0, None),
            (id2, "en2", 0, None),
        ]
        with override("de"):
        ...
        assert list(
            manager.values_list("title", flat=True).annotate(Count("title"))
        ) == ["en"]

        # custom annotation
        assert (
            list(manager.filter(id=id1).annotate(custom_id=F("id")).values_list())[0][
                -1
            ]
            == id1
        )
        assert (
            list(manager.filter(id=id1).annotate(custom_id=F("id")).values())[0].get(
                "custom_id"
            )
            == id1
        )
        ...
        assert list(
            manager.filter(id=id1).annotate(custom_id=F("id")).values_list("id")
        )[0] == (id1,)
        assert (
            list(manager.filter(id=id1).annotate(custom_id=F("id")).values("id"))[
                0
            ].get("custom_id")
            is None
        )
    ...
    def test_values_list_annotation(self):
        ...
        models.TestModel(title="foo").save()
        assert list(
            models.TestModel.objects.all().values_list("title").annotate(Count("id"))
        ) == [("foo", 2)]
    ...
    def test_values_with_expressions(self):
        ...
        raw_obj = manager.raw_values(
            "title", str_pk=Cast("pk", output_field=CharField())
        )[0]
        obj = manager.values("title", str_pk=Cast("pk", output_field=CharField()))[0]
        with override("de"):
            raw_obj2 = manager.raw_values(
                "title", str_pk=Cast("pk", output_field=CharField())
            )[0]
            obj2 = manager.values("title", str_pk=Cast("pk", output_field=CharField()))[
                0
            ]
        ...
        # Values_list behave similarly
        assert list(
            manager.values_list("title", Cast("pk", output_field=CharField()))
        ) == [("en", str(id1))]
        with override("de"):
            assert list(
                manager.values_list("title", Cast("pk", output_field=CharField()))
    ...
    def test_default_manager_for_inherited_models_with_custom_manager(self):
        ...
        assert isinstance(manager, MultilingualManager)
        assert isinstance(
            models.CustomManagerChildTestModel.translations, MultilingualManager
        )
    ...
    def test_creation_population(self):
        ...
        n = models.ManagerTestModel.objects.populate(True).create(
            title="foo", title_de="bar"
        )
        assert "foo" == n.title_en
    ...
    def assertDeferred(self, use_defer, *fields):
        manager = (
            models.TestModel.objects.defer
            if use_defer
            else models.TestModel.objects.only
        )
    ...
    def test_deferred_fk(self):
        ...
        item = models.ForeignKeyModel.objects.select_related("test").defer(
            "test__text"
        )[0]
    ...
    def test_deferred_spanning(self):
        ...
        item1 = (
            models.ForeignKeyModel.objects.select_related("test")
            .defer("test__text")[0]
            .test
        )
    ...
    def test_select_related(self):
        ...
        assert (
            "untrans"
            not in fk_qs.select_related("untrans")
            .select_related(None)[0]
            ._state.fields_cache
        )
...
class TranslationModelFormTest(ModeltranslationTestBase):
    ...
    def test_updating_with_empty_value(self):
        ...
        form = Form(
            {"text_de": "", "title": "a", "email_de": "", "email_en": ""},
            instance=instance,
        )
        instance = form.save()
        assert "de" == get_language()
