...
from ._compat import clear_ForeignObjectRel_caches, is_hidden
...
def create_translation_field(
    model: type[Model], field_name: str, lang: str, empty_value: Any
):
    """
    ...
    if not (
        isinstance(field, SUPPORTED_FIELDS) or cls_name in mt_settings.CUSTOM_FIELDS
    ):
    ...
    return translation_class(
        translated_field=field, language=lang, empty_value=empty_value
    )
...
class TranslationField:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if isinstance(
            self.translated_field, fields.related.ManyToManyField
        ) and hasattr(self.remote_field, "through"):
            ...
            if self.remote_field.symmetrical and (
                ...
                if self.remote_field.related_name is None:
                    ...
                    self.remote_field.related_name = "%s_set" % (
                        build_localized_fieldname(
                            self.model.__name__.lower(), language
                        ),
                    )
                else:
    ...
    def __eq__(self, other: object) -> bool:
        if isinstance(other, fields.Field):
            return (
                ...
                and self.language == getattr(other, "language", None)
            )
    ...
    def save_form_data(self, instance: Model, data: Any, check: bool = True) -> None:
        ...
        if (
            check
            and self.language == get_language()
...
class TranslationFieldDescriptor:
    ...
    def __set__(self, instance, value):
        ...
        if isinstance(self.field, fields.related.ForeignKey):
            instance.__dict__[self.field.get_attname()] = (
                None if value is None else value.pk
            )
        if getattr(instance, "_mt_init", False) or getattr(
            instance, "_mt_disable", False
        ):
