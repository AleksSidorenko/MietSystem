...
from modeltranslation._typing import AutoPopulate, Self
...
def append_fallback(
    model: type[Model], fields: Sequence[str]
...
class MultilingualQuerySet(QuerySet[_T]):
    ...
    def _rewrite_order(self) -> None:
        self.query.order_by = [
            rewrite_order_lookup_key(self.model, field_name)
    ...
    def _update(
        self, values: list[tuple[Field, type[Model] | None, Any]]
    ) -> CursorWrapper:
        """
        ...
        for field, model, value in values:
            ...
            if translation_field_name not in field_names_to_update:
                translatable_field = cast(
                    Field, self.model._meta.get_field(translation_field_name)
                )
    ...
    def _values(self, *original: str, **kwargs: Any) -> Self:
        ...
        annotation_keys = (
            set(self.query.annotation_select.keys()) if selects_all else set()
        )
    ...
    def values(self, *fields: str, **expressions: Any) -> Self:
        ...
        clone = self._values(
            *fields, prepare=True, selects_all=selects_all, **expressions
        )
    ...
    def values_list(
        ...
        clone = self._values(
            *_fields, prepare=True, selects_all=selects_all, **expressions
        )
        clone._iterable_class = (
            FallbackNamedValuesListIterable
            if named
            else FallbackFlatValuesListIterable if flat else FallbackValuesListIterable
        )
        return clone
...
class FallbackValuesIterable(ValuesIterable):
    ...
    def __iter__(self) -> Iterator[dict[str, Any]]:
        ...
        fields += tuple(
            f for f in self.queryset.query.annotation_select if f not in fields
        )
