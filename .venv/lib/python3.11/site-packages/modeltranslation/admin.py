...
class TranslationBaseModelAdmin(BaseModelAdmin[_ModelT]):
    ...
    def _get_declared_fieldsets(
        ...
        if fieldsets:
            ...
            return [
                (
                    None,
                    {"fields": self.replace_orig_field(self.get_fields(request, obj))},
                )
            ]
        return None
    ...
    def patch_translation_field(
        ...
        if db_field.name in self.trans_opts.all_fields:
            if field.required:
                ...
                self._orig_was_required[
                    "%s.%s" % (db_field.model._meta, db_field.name)
                ] = True
        ...
        try:
            ...
            try:
                ...
                if isinstance(field.widget, type):  # if not initialized
                    field.widget = field.widget(
                        attrs
                    )  # initialize form widget with attrs
            ...
            if (
                db_field.empty_value == "both"
                or orig_field.name in self.both_empty_values_fields
            ) and isinstance(field.widget, (forms.TextInput, forms.Textarea)):
                field.widget = ClearableWidgetWrapper(field.widget)
            css_classes = (
                self._get_widget_from_field(field).attrs.get("class", "").split(" ")
            )
    ...
    def _exclude_original_fields(
        self, exclude: _ListOrTuple[str] | None = None
    ) -> tuple[str, ...]:
        if exclude is None:
            exclude = tuple()
        if exclude:
    ...
    def replace_orig_field(
        self, option: Iterable[str | Sequence[str]]
    ) -> _ListOrTuple[str]:
        """
...
class TranslationInlineModelAdmin(
    TranslationBaseModelAdmin[_ChildModelT],
    InlineModelAdmin[_ChildModelT, _ParentModelT],
):
    def get_formset(
