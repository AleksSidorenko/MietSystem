...
class Environment:
    ...
    def create_master_runner(
        self, master_bind_host="*", master_bind_port=5557
    ) -> MasterRunner:
        """
    ...
    def update_user_class(self, user_settings):
        ...
        for key, value in user_settings.items():
            ...
            if key == "tasks":
                user_class.tasks = [
                    task for task in user_tasks if task.__name__ in value
                ]

    def update_worker_logs(self, worker_log_report):
        if worker_log_report.get("worker_id", None):
            self.worker_logs[worker_log_report.get("worker_id")] = (
                worker_log_report.get("logs", [])
            )
    ...
    def _filter_tasks_by_tags(self) -> None:
        ...
        elif self.parsed_options and getattr(
            self.parsed_options, "exclude_tags", False
        ):
    ...
    def _validate_shape_class_instance(self):
        if self.shape_class is not None and not isinstance(
            self.shape_class, LoadTestShape
        ):
            raise ValueError(
