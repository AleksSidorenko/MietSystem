...
class LocustArgumentParser(configargparse.ArgumentParser):
    ...
    def args_included_in_web_ui(self) -> dict[str, configargparse.Action]:
        return {
            ...
            if hasattr(a, "include_in_web_ui") and a.include_in_web_ui
        }
    ...
    def secret_args_included_in_web_ui(self) -> dict[str, configargparse.Action]:
        return {
            a.dest: a
            for a in self._actions
            if a.dest in self.args_included_in_web_ui
            and hasattr(a, "is_secret")
            and a.is_secret
        }
...
def download_locustfile_from_url(url: str) -> str:
    ...
    try:
        ...
        sys.stderr.write(
            f"Failed to get locustfile from: {url}. Response is not valid python code."
        )
        sys.exit(1)

    with open(
        os.path.join(tempfile.gettempdir(), url.rsplit("/", 1)[-1]), "w"
    ) as locustfile:
...
def download_locustfile_from_master(master_host: str, master_port: int) -> str:
    ...
    try:
        ...
        msg = gevent.spawn(wait_for_reply).get(
            timeout=runners.CONNECT_TIMEOUT * runners.CONNECT_RETRY_COUNT
        )
        got_reply = True
...
def parse_locustfiles_from_master(locustfile_sources) -> list[str]:
    ...
    for source in locustfile_sources:
        if "contents" in source:
            ...
            with open(
                os.path.join(tempfile.gettempdir(), filename), "w", encoding="utf-8"
            ) as locustfile:
