...
class Runner:
    ...
    def __init__(self, environment: Environment) -> None:
        ...
        self.shape_last_tick: (
            tuple[int, float] | tuple[int, float, list[type[User]] | None] | None
        ) = None
        ...
        self.greenlet.spawn(self.monitor_cpu_and_memory).link_exception(
        ...
        self.final_user_classes_count: dict[str, int] = (
            {}
        )  # just for the ratio report, fills before runner stops
    ...
    def user_classes_count(self) -> dict[str, int]:
        ...
        """
        user_classes_count = {
            user_class.__name__: 0 for user_class in self.user_classes
        }
    ...
    def stop_users(self, user_classes_stop_count: dict[str, int]) -> None:
        ...
        logger.debug(
            "%g users have been stopped, %g still running",
            sum(user_classes_stop_count.values()),
            self.user_count,
        )
    ...
    def monitor_cpu_and_memory(self) -> NoReturn:
        ...
        while True:
            ...
            if self.current_cpu_usage > CPU_WARNING_THRESHOLD:
                self.environment.events.cpu_warning.fire(
                    environment=self.environment, cpu_usage=self.current_cpu_usage
                )
    ...
    @abstractmethod
    def send_message(
        self, msg_type: str, data: Any | None = None, client_id: str | None = None
    ) -> None: ...
    ...
    def shape_worker(self) -> None:
        ...
        while (
            ...
            current_tick = (
                self.environment.shape_class.tick()
                if self.environment.shape_class is not None
                else None
            )
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Stopping all users (called from {caller.filename}:{caller.lineno})"
            )
        except Exception:
            logger.debug(
                "Stopping all users (couldn't determine where stop() was called from)"
            )
    ...
    def register_message(
        self, msg_type: str, listener: Callable, concurrent=False
    ) -> None:
        """
        ...
        if msg_type in self.custom_messages:
            raise Exception(
                f"Tried to register listener method for {msg_type}, but it already had a listener!"
            )
...
class LocalRunner(Runner):
    ...
    def _start(
        ...
        logger.info(
            "Ramping to %d users at a rate of %.2f per second"
            % (user_count, spawn_rate)
        )
        ...
        try:
            for dispatched_users in self._users_dispatcher:
                ...
                logger.debug(
                    "Ramping to %s"
                    % _format_user_classes_count_for_log(user_classes_count)
                )
                ...
                self._local_worker_node.user_classes_count = next(
                    iter(dispatched_users.values())
                )
        ...
        logger.info(
            f"All users spawned: {_format_user_classes_count_for_log(self.user_classes_count)}"
        )
    ...
    def start(
        ...
        self.spawning_greenlet = self.greenlet.spawn(
            lambda: self._start(
                user_count, spawn_rate, wait=wait, user_classes=user_classes
            )
        )
...
class MasterRunner(DistributedRunner):
    ...
    def __init__(self, environment, master_bind_host, master_bind_port) -> None:
        ...
        try:
            ...
            if e.args[0] == "Socket bind failure: Address already in use":
                port_string = (
                    master_bind_host + ":" + str(master_bind_port)
                    if master_bind_host != "*"
                    else str(master_bind_port)
                )
                logger.error(
    ...
    def start(
        ...
        try:
            for dispatched_users in self._users_dispatcher:
                ...
                for (
                    worker_node_id,
                    worker_user_classes_count,
                ) in dispatched_users.items():
                    data = {
                        "timestamp": time.time(),
                        ...
                        "parsed_options": (
                            vars(self.environment.parsed_options)
                ...
                dispatched_user_count = sum(
                    map(sum, map(methodcaller("values"), dispatched_users.values()))
                )
                logger.debug(
            ...
            self.target_user_classes_count = _aggregate_dispatched_users(
                dispatched_users
            )
        ...
        try:
            ...
            msg_prefix = "Spawning is complete and report waittime is expired, but not all reports received from workers"
        finally:
            timeout.cancel()
        ...
        logger.info(
            f"{msg_prefix}: {_format_user_classes_count_for_log(self.reported_user_classes_count)}"
        )
    ...
    def _wait_for_workers_report_after_ramp_up(self) -> float:
        ...
        """
        locust_wait_for_workers_report_after_ramp_up = os.getenv(
    ...
    def quit(self) -> None:
        ...
        for client in self.clients.all:
            logger.debug(
                f"Sending quit message to worker {client.id} (index {self.get_worker_index(client.id)})"
            )
    ...
    def check_stopped(self) -> None:
        if (
            ...
            or all(
                x.state not in (STATE_RUNNING, STATE_SPAWNING, STATE_INIT)
                for x in self.clients.all
            )
        ):
            self.update_state(STATE_STOPPED)
        ...
        while True:
            ...
            for client in self.clients.all:
                ...
                if (
                    ...
                    and client.heartbeat <= HEARTBEAT_DEAD_INTERNAL
                ):
                    missing_clients_to_be_removed.append(client.id)

                if client.heartbeat < 0 and client.state != STATE_MISSING:
                    logger.info(
                        f"Worker {str(client.id)} failed to send heartbeat, setting state to missing."
                    )
                    client.state = STATE_MISSING
            ...
            if len(missing_clients_to_be_removed) > 0:
                ...
                if self.state == STATE_RUNNING or self.state == STATE_SPAWNING:
                    ...
                    self.start(
                        user_count=self.target_user_count, spawn_rate=self.spawn_rate
                    )
    ...
    def reset_connection(self) -> None:
        ...
        try:
            ...
            logger.error(
                f"Temporary failure when resetting connection: {e}, will retry later."
            )
        ...
        while True:
            try:
                ...
                if client_id and client_id in self.clients:
                    logger.error(
                        f"RPCError when receiving from client: {e}. Will reset client {client_id}."
                    )
                    try:
                        self.server.send_to_client(
                            Message("reconnect", None, client_id)
                        )
                    except Exception as error:
                        logger.error(
                            f"Error sending reconnect message to worker: {error}. Will reset RPC server."
                        )
                ...
                logger.error(
                    f"Error sending reconnect message to worker: {e}. Will reset RPC server."
                )
                self.connection_broken = True
                gevent.sleep(FALLBACK_INTERVAL)
                continue
            ...
            if msg.type == "client_ready":
                if not msg.data:
                    logger.error(
                        f"An old (pre 2.0) worker tried to connect ({client_id}). That's not going to work."
                    )
                    continue
                ...
                self.send_message(
                    "ack",
                    client_id=client_id,
                    data={"index": self.get_worker_index(client_id)},
                )
                self.environment.events.worker_connect.fire(client_id=msg.node_id)
                self.clients[client_id] = WorkerNode(
                    client_id, heartbeat_liveness=HEARTBEAT_LIVENESS
                )
                if self._users_dispatcher is not None:
                    self._users_dispatcher.add_worker(
                        worker_node=self.clients[client_id]
                    )
                    if (
                        not self._users_dispatcher.dispatch_in_progress
                        and self.state == STATE_RUNNING
                    ):
                ...
                locustfile_list = [
                    f.strip() for f in locustfile_options if not os.path.isdir(f)
                ]

                for locustfile_option in locustfile_options:
                    if os.path.isdir(locustfile_option):
                        locustfile_list.extend(
                            get_abspaths_in(locustfile_option, extension=".py")
                        )
                ...
                try:
                    ...
                    for filename in locustfile_list:
                        if is_url(filename):
                            ...
                            locustfiles.append(
                                {"filename": filename, "contents": file_contents}
                            )
                ...
                if msg.node_id not in self.clients:
                    logger.warning(
                        f"Received {msg.type} message from an unknown worker: {msg.node_id}."
                    )
                    continue
                ...
                if msg.node_id in self.clients:
                    ...
                    if c.state == STATE_MISSING:
                        logger.info(
                            f"Worker {str(c.id)} self-healed with heartbeat, setting state to {client_state}."
                        )
                        if self._users_dispatcher is not None:
                    ...
                    self.environment.events.heartbeat_sent.fire(
                        client_id=msg.node_id, timestamp=time.time()
                    )
                    self.server.send_to_client(Message("heartbeat", None, msg.node_id))
                else:
                    logging.debug(
                        f"Got heartbeat message from unknown worker {msg.node_id}"
                    )
            elif msg.type == "stats":
                ...
                try:
                    ...
                    logger.warning(
                        f"Got spawning message from unknown worker {msg.node_id}. Asking worker to quit."
                    )
                ...
                self.clients[msg.node_id].user_classes_count = msg.data[
                    "user_classes_count"
                ]
            elif msg.type == "logs":
                ...
                try:
                    ...
                    logging.error(
                        f"Uncaught exception in handler for {msg.type}\n{traceback.format_exc()}"
                    )
...
class WorkerRunner(DistributedRunner):
    ...
    def __init__(
        self, environment: Environment, master_host: str, master_port: int
    ) -> None:
        """
        :param environment: Environment instance
        ...
        self.greenlet.spawn(self.heartbeat_timeout_checker).link_exception(
            greenlet_exception_handler
        )
        self.greenlet.spawn(self.stats_reporter).link_exception(
            greenlet_exception_handler
        )
        self.greenlet.spawn(self.logs_reporter).link_exception(
            greenlet_exception_handler
        )
    ...
    def heartbeat_timeout_checker(self) -> NoReturn:
        while True:
            ...
            if (
                ...
                and self.last_heartbeat_timestamp
                < time.time() - MASTER_HEARTBEAT_TIMEOUT
            ):
                logger.error(
                    f"Didn't get heartbeat from master in over {MASTER_HEARTBEAT_TIMEOUT}s"
                )
                self.quit()
    ...
    def worker(self) -> NoReturn:
        ...
        while True:
            ...
            if msg.type == "ack":
                ...
                if (
                    self.worker_state != STATE_RUNNING
                    and self.worker_state != STATE_SPAWNING
                ):
                    self.stats.clear_all()
                    self.exceptions = {}
                    self.cpu_warning_emitted = False
                ...
                if self.spawning_greenlet:
                    ...
                    self.spawning_greenlet.kill(block=True)
                self.spawning_greenlet = self.greenlet.spawn(
                    lambda: self.start_worker(job["user_classes_count"])
                )
                self.spawning_greenlet.link_exception(greenlet_exception_handler)
                ...
                additional_wait = int(
                    os.getenv(
                        "LOCUST_WORKER_ADDITIONAL_WAIT_BEFORE_READY_AFTER_STOP", 0
                    )
                )
                ...
                logger.warning(
                    "Received reconnect message from master. Resetting RPC connection."
                )
                self.reset_connection()
            elif msg.type == "heartbeat":
                ...
                self.environment.events.spawning_complete.fire(
                    user_count=msg.data["user_count"]
                )
            elif msg.type in self.custom_messages:
    ...
    def stats_reporter(self) -> NoReturn:
        while True:
            try:
                ...
                logger.error(
                    f"Temporary connection lost to master server: {e}, will retry later."
                )
