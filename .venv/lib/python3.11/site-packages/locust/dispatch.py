...
class UsersDispatcher(Iterator):
    ...
    def __init__(self, worker_nodes: list[WorkerNode], user_classes: list[type[User]]):
        ...
        self._initial_users_on_workers = {
            worker_node.id: {
                user_class.__name__: 0 for user_class in self._user_classes
            }
            for worker_node in worker_nodes
        }

        self._users_on_workers = self._fast_users_on_workers_copy(
            self._initial_users_on_workers
        )
    ...
    def _sort_workers(self):
        ...
        self._worker_nodes = sorted(
            self._worker_nodes,
            key=lambda worker: (worker._index_within_host, worker.id),
        )
    ...
    def new_dispatch(
        ...
        """
        if user_classes is not None and self._user_classes != sorted(
            user_classes, key=attrgetter("__name__")
        ):
            self._user_classes = sorted(user_classes, key=attrgetter("__name__"))
            self._user_generator = self._user_gen()
        ...
        self._wait_between_dispatch = (
            self._user_count_per_dispatch_iteration / self._spawn_rate
        )
    ...
    def _prepare_rebalance(self) -> None:
        ...
        self._users_on_workers = {
            worker_node.id: {
                user_class.__name__: 0 for user_class in self._original_user_classes
            }
            for worker_node in self._worker_nodes
        }
        self._try_dispatch_fixed = True

        users_on_workers, user_gen, worker_gen, active_users = self._distribute_users(
    ...
    def _add_users_on_workers(self) -> dict[str, dict[str, int]]:
        ...
        current_user_count_target = min(
            self._current_user_count + self._user_count_per_dispatch_iteration,
            self._target_user_count,
        )
    ...
    def _remove_users_from_workers(self) -> dict[str, dict[str, int]]:
        ...
        while True:
            ...
            if (
                ...
                or self._current_user_count <= current_user_count_target
            ):
                return self._users_on_workers
    ...
    def _distribute_users(self, target_user_count: int) -> tuple[
        dict[str, dict[str, int]],
        Iterator[str | None],
        itertools.cycle,
        list[tuple[WorkerNode, str]],
    ]:
        """
    ...
    def _user_gen(self) -> Iterator[str | None]:
        weighted_users_gen = _kl_generator(
            (u.__name__, u.weight) for u in self._user_classes if not u.fixed_count
        )

        while True:
            if (
                self._try_dispatch_fixed
            ):  # Fixed_count users are spawned before weight users.
                ...
                fixed_users_missing = [
                    ...
                    if u.fixed_count
                    and (
                        miss := u.fixed_count - self._get_user_current_count(u.__name__)
                    )
                    > 0
                ]
    ...
    @staticmethod
    def _fast_users_on_workers_copy(
        ...
        """
        return dict(
            zip(users_on_workers.keys(), map(dict.copy, users_on_workers.values()))
        )
