...
def calculate_response_time_percentile(
    response_times: dict[int, int], num_requests: int, percent: float
) -> int:
    """
...
def diff_response_time_dicts(
    latest: dict[int, int], old: dict[int, int]
) -> dict[int, int]:
    """
...
class RequestStats:
    ...
    def serialize_stats(self) -> list[StatsEntryDict]:
        return [
            ...
            if not (e.num_requests == 0 and e.num_failures == 0)
        ]
...
class StatsEntry:
    ...
    def extend(self, other: StatsEntry) -> None:
        ...
        if self.use_response_times_cache:
            ...
            last_time = (
                int(self.last_request_timestamp)
    ...
    def serialize(self) -> StatsEntryDict:
        return cast(
            StatsEntryDict,
            {
                ...
                for key in StatsEntryDict.__annotations__.keys()
            },
        )
    ...
    def to_dict(self, escape_string_values=False):
        response_time_percentiles = {
            f"response_time_percentile_{percentile}": self.get_response_time_percentile(
                percentile
            )
        ...
        return {
            "method": (
                escape(self.method or "") if escape_string_values else self.method
            ),
            ...
            "min_response_time": (
                0
                if self.min_response_time is None
                else proper_round(self.min_response_time)
            ),
...
class StatsError:
    ...
    def to_name(self) -> str:
        ...
        if isinstance(
            error, str
        ):  # in distributed mode, all errors have been converted to strings
    ...
    def serialize(self) -> StatsErrorDict:
        ...
        return cast(
            StatsErrorDict,
            {
                key: _getattr(self, key, None)
                for key in StatsErrorDict.__annotations__.keys()
            },
        )
...
def setup_distributed_stats_event_listeners(
...
def get_stats_summary(stats: RequestStats, current=True) -> list[str]:
    ...
    """
    name_column_width = (
        STATS_NAME_WIDTH - STATS_TYPE_WIDTH
    ) + 4  # saved characters by compacting other columns
    summary = []
    summary.append(
        (
            "%-"
...
def update_stats_history(runner: Runner) -> None:
    ...
    r = {
        **current_response_time_percentiles,
        ...
        "total_avg_response_time": proper_round(
            stats.total.avg_response_time, digits=2
        ),
...
class StatsCSV:
    ...
    def _percentile_fields(
        ...
        if not stats_entry.num_requests:
            ...
            return [
                int(stats_entry.get_current_response_time_percentile(x) or 0)
    ...
    def _exceptions_data_rows(self, csv_writer: CSVWriter) -> None:
        ...
        for exc in self.environment.runner.exceptions.values():
            csv_writer.writerow(
                [exc["count"], exc["msg"], exc["traceback"], ", ".join(exc["nodes"])]
            )
...
class StatsCSVFileWriter(StatsCSV):
    ...
    def __init__(
        ...
        self.exceptions_csv_filehandle = open(
            self.base_filepath + "_exceptions.csv", "w"
        )
