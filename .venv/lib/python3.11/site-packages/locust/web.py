...
class WebUI:
    ...
    def __init__(
        ...
        def logs():
            return jsonify(
                {"master": get_logs(), "workers": self.environment.worker_logs}
            )
        ...
        if self.tls_cert and self.tls_key:
            ...
            class RewriteFilter(logging.Filter):
                def filter(self, record) -> bool:
                    msg = record.msg
                    if (
                        "gevent._socket3.socket at" in msg
                        and "Invalid HTTP method: '\x16\x03" in msg
                    ):
            ...
            self.server = pywsgi.WSGIServer(
                (self.host, self.port), self.app, log=None, error_log=logger
            )
    ...
    def update_template_args(self):
        ...
        if self.environment.available_shape_classes:
            available_shape_classes += sorted(
                self.environment.available_shape_classes.keys()
            )
        ...
        available_user_tasks = (
            {
                ...
                for (
                    user_class_name,
                    user_class,
                ) in self.environment.available_user_tasks.items()
            }
        ...
        self.template_args = {
            ...
            "hide_common_options": (
                ...
                and not (
                    self.userclass_picker_is_active
                    or self.environment.shape_class.use_common_options
                )
            ),
    ...
    def _update_user_classes(self, user_classes):
        ...
        if self.environment.locustfile is None:
            self.environment.locustfile = ",".join(
                self.environment.user_classes_by_name.keys()
            )
