...
def is_user_class(item):
    ...
    """
    return bool(
        inspect.isclass(item) and issubclass(item, User) and item.abstract is False
    )
...
def is_shape_class(item):
    ...
    return bool(
        ...
        and not getattr(item, "abstract", True)
    )
...
def load_locustfile(path) -> tuple[str | None, dict[str, User], list[LoadTestShape]]:
    ...
    shape_classes = [
        value() for value in vars(imported).values() if is_shape_class(value)
    ]
