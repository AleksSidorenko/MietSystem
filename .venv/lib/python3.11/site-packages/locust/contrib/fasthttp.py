...
class FastHttpSession:
    ...
    def request(
        ...
        **kwargs,
        ...
        response = self._send_request_safe_mode(
            method, built_url, payload=data, headers=headers, **kwargs
        )
        request_meta = {
            "request_type": method,
            "name": name or url,
        ...
        request_meta["response_length"] = (
            0  # default value, if length cannot be determined
        )
        ...
        if stream:
            if response.headers and "response_length" in response.headers:
                request_meta["response_length"] = int(
                    response.headers["response_length"]
                )
        else:
            try:
                request_meta["response_length"] = (
        ...
        if catch_response:
            return ResponseContextManager(
                response, environment=self.environment, request_meta=request_meta
            )
        else:
            try:
    ...
    def delete(
        self, url: str, **kwargs: Unpack[RESTKwargs]
    ) -> ResponseContextManager | FastResponse:
...
class FastHttpUser(User):
    ...
    def rest(
        ...
        * Catches any unhandled exceptions thrown inside your with-block, marking the sample as failed (instead of exiting the task immediately without even firing the request event)
        ...
        with self.client.request(
            method, url, catch_response=True, headers=headers, **kwargs
        ) as r:
            ...
            try:
                ...
                for l in traceback.format_exc().split("\n"):
                    ...
                    short_resp = resp.text[:200] if resp.text else resp.text
                    resp.failure(
                        f"{e.__class__.__name__}: {e} at {', '.join(error_lines)}. Response was {short_resp}"
                    )
    ...
    def rest_(
        ...
        with self.rest(
            method, f"{url}{separator}_={int(time.time()*1000)}", name=name, **kwargs
        ) as resp:
            yield resp
...
class LocustUserAgent(UserAgent):
    ...
    valid_response_codes = frozenset(
        [200, 201, 202, 203, 204, 205, 206, 207, 208, 226, 301, 302, 303, 304, 307]
    )
