...
def main():
    ...
    if len(stats.PERCENTILES_TO_CHART) > 6:
        logging.error(
            "stats.PERCENTILES_TO_CHART parameter should be a maximum of 6 parameters \n"
        )
        sys.exit(1)
    ...
    if options.autoquit != -1 and not options.autostart:
        sys.stderr.write(
            "--autoquit is only meaningful in combination with --autostart\n"
        )
        sys.exit(1)
    ...
    if options.legacy_ui:
        sys.stderr.write(
            "--legacy-ui is no longer supported, remove the parameter to continue\n"
        )
        sys.exit(1)
            ...
            if options.worker:
                ...
                def sigint_handler(_signal, _frame):
                    if getattr(sigint_handler, "has_run", False):
                        ...
                        for child_pid in children:
                            try:
                                logging.debug(
                                    f"Sending SIGKILL to child with pid {child_pid}"
                                )
                ...
                def kill_workers(children):
                    ...
                    for child_pid in children:
                        try:
                            logging.debug(
                                f"Sending SIGINT to child with pid {child_pid}"
                            )
                    ...
                    if exit_code > 1:
                        logging.error(
                            f"Bad response code from worker children: {exit_code}"
                        )
    ...
    if options.stop_timeout:
        try:
            ...
            logger.error(
                "Valid --stop-timeout formats are: 20, 20s, 3m, 2h, 1h20m, 3h30m10s, etc."
            )
            sys.exit(1)
    ...
    if os.name != "nt" and not options.master:
        try:
            ...
            if soft_limit < minimum_open_file_limit:
                ...
                resource.setrlimit(
                    resource.RLIMIT_NOFILE, [minimum_open_file_limit, hard_limit]
                )
...
See https://github.com/locustio/locust/wiki/Installation#increasing-maximum-number-of-open-files-limit for more info."""
    ...
    if options.config_users:
        for json_user_config in options.config_users:
            try:
                ...
                logger.error(
                    f"The --config-users argument must be in valid JSON string or file: {e}"
                )
                sys.exit(-1)
    ...
    if (
        ...
        ignored = [
            f"--{arg}"
            for opt, arg in COMMON_OPTIONS.items()
    ...
    if options.master:
        ...
        if options.expect_workers < 1:
            logger.error(
                f"Invalid --expect-workers argument ({options.expect_workers}), must be a positive number"
            )
            sys.exit(-1)
        ...
        try:
            ...
            logger.debug(
                "Connected to locust master: %s:%s",
                options.master_host,
                options.master_port,
            )
        except OSError as e:
    ...
    if not options.headless and not options.worker:
        ...
        if web_host:
            logger.info(
                f"Starting web interface at {protocol}://{web_host}:{options.web_port}"
            )
        else:
            if os.name == "nt":
                logger.info(
                ...
                logger.info(
                    f"Starting web interface at {protocol}://0.0.0.0:{options.web_port}"
                )
    ...
    if options.autostart and options.headless:
        logger.warning(
            "The --autostart argument is implied by --headless, no need to set both."
        )
    ...
    def stop_and_optionally_quit():
        if options.autostart and not options.headless:
            ...
            if options.autoquit != -1:
                ...
                logger.info(
                    "--autoquit not specified, leaving web ui running indefinitely"
                )
        else:  # --headless run
    ...
    def spawn_run_time_quit_greenlet():
        gevent.spawn_later(options.run_time, stop_and_optionally_quit).link_exception(
    ...
    def start_automatic_run():
        if options.master:
            ...
            while len(runner.clients.ready) < options.expect_workers:
                if (
                    options.expect_workers_max_wait
                    and options.expect_workers_max_wait < time.monotonic() - start_time
                ):
        ...
        if not options.worker:
            ...
            if environment.shape_class:
                ...
                headless_master_greenlet = gevent.spawn(
            ...
            input_listener(
                {
                    "w": lambda: (
                        ...
                        if runner.state != "spawning"
                            ...
                            "Already spawning users, can't spawn more right now"
                        )
                    ),
                    "W": lambda: (
