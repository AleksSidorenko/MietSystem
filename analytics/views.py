...
class AnalyticsViewSet(viewsets.ModelViewSet):
    ...
    def top_5(self, request):
        ...
        if time_frame not in ["week", "month"]:
            return Response({"error": _("Invalid time frame. Use 'week' or 'month'")}, status=status.HTTP_400_BAD_REQUEST)
...
class SearchHistoryViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if user:
            # Оставляем в истории поиска не больше 10 последних записей на пользователя
