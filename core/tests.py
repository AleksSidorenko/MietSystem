...
class HealthCheckTests(TestCase):
    ...
    def test_language_switch(self):
        ...
        response = self.client.post('/i18n/set_language/', {
            'language': 'ru',
            'next': reverse('status')
        })
    ...
    def test_system_metrics(self):
        response = self.client.get(reverse('status'))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['data']['system_metrics']['disk_free_gb'], 100)
    ...
    def test_localization_status(self):
        ...
        self.assertEqual(response.context['data']['localization']['languages'], ['en', 'ru', 'de'])
